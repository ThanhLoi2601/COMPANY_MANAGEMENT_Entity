    ...
    class JobDAO
    {
        ...
        public void Insert(Job job)
        {
            string sqlStr = string.Format("INSERT INTO Job(ID,Name,Content,DateStart,DateEnd,Bonus,IDTasks) values ('{0}','{1}','{2}','{3}','{4}','{5}','{6}')"
                , job.ID, job.Name, job.Content,job.DateStart?.ToString("yyyy-MM-dd"),job.DateEnd?.ToString("yyyy-MM-dd"),job.Bonus.ToString(),job.IDTasks);
            dB.Executive(sqlStr);
        }
        ...
        public void Update(Job job)
        {
            string sqlStr = string.Format("UPDATE Job Set Name = '{0}', Content = '{1}', DateStart = '{2}', DateEnd = '{3}', Bonus = {4}, IDTasks= '{5}' WHERE ID ='{6}'"
                , job.Name,job.Content,job.DateStart?.ToString("yyyy-MM-dd"),job.DateEnd?.ToString("yyyy-MM-dd"),job.Bonus,job.IDTasks,job.ID);
            dB.Executive(sqlStr);
        }
